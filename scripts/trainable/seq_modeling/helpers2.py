...
#     pickle.dump(dict(i2w), open("vocab/" + task_name + "i2w_" + str(WORD_LIMIT) + \
#       ".p", 'wb')) # don't think its needed
...
def _get_line_representation(
    line, rep_list=["swap", "drop", "add", "key"], probs=[0.25, 0.25, 0.25, 0.25]
):
    modified_words = []
    for word in line.split():
...
def get_line_representation(
    lines, rep_list=["swap", "drop", "add", "key"], probs=[0.25, 0.25, 0.25, 0.25]
):
    # rep = []

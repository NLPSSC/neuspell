...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 4:
        ...
        if FINETUNE == "probword":
            SRC_CHECKPOINT_PATH = os.path.join(
                BASE_PATH, "checkpoints/scrnnelmo-probwordnoise"
            )
    ...
    if TRAIN_NOISE_TYPE == "word":
        ...
        CHECKPOINT_PATH = os.path.join(
            BASE_PATH, "checkpoints/scrnnelmo-probwordnoise-bea40kfinetune"
        )
    elif TRAIN_NOISE_TYPE == "moviereviewsfinetune":
        CHECKPOINT_PATH = os.path.join(
            BASE_PATH, "checkpoints/scrnnelmo-probwordnoise-moviereviewsfinetune2"
        )
...
#                     input_ids = torch.tensor(gpt2Tokenizer.encode(sent, add_special_tokens=True))
...
# this_batch = [[k_batch_predictions[k][i] for k in range(len(k_batch_predictions))] for i in

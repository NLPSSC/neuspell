...
def model_predictions(model, data, vocab, DEVICE, BATCH_SIZE=16):
    ...
    for batch_id, (batch_clean_sentences, batch_corrupt_sentences) in enumerate(
        ...
        batch_predictions = untokenize_without_unks(
            batch_predictions, batch_lengths, vocab, batch_clean_sentences
        )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 4:
        ...
        if FINETUNE == "probword":
            SRC_CHECKPOINT_PATH = os.path.join(
                BASE_PATH, "checkpoints/scrnn-probwordnoise"
            )
    ...
    if TRAIN_NOISE_TYPE == "word":
        ...
        CHECKPOINT_PATH = os.path.join(
            BASE_PATH, "checkpoints/scrnn-probwordnoise-bea40kfinetune"
        )
    elif TRAIN_NOISE_TYPE == "moviereviewsfinetune":
        CHECKPOINT_PATH = os.path.join(
            BASE_PATH, "checkpoints/scrnn-probwordnoise-moviereviewsfinetune2"
        )
    ...
    if START_EPOCH > 0:
        ...
        for epoch_id in range(START_EPOCH, N_EPOCHS + 1):
            ...
            for batch_id, (batch_labels, batch_sentences) in enumerate(train_data_iter):
                ...
                if batch_id % 10000 == 0:
                    ...
                    with torch.no_grad():
                        _, batch_predictions = model(
                            batch_idxs, batch_lengths, targets=batch_labels
                        )
                    model.train()
                    batch_labels = batch_labels.cpu().detach().numpy()
                    batch_lengths = batch_lengths.cpu().detach().numpy()
        ...
        for x, y, z in zip(paths, files1, files2):
            ...
            greedy_results = model_inference(
                model, test_data, topk=1, DEVICE=DEVICE, BATCH_SIZE=INFER_BATCH_SIZE
            )
        print(ANALYSIS_DIR)

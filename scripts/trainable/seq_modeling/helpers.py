...
def untokenize_without_unks3(
    ...
    """
    batch_predictions are indices of vocab that have the top values at each timestep and
        should have shape (batch_size,max_seq_len,vocab_size)
    batch_predictions_probs are the corresponding softmaxed probabilities and should also
        have shape (batch_size,max_seq_len,vocab_size)
    batch_lengths should have shape (batch_size)
    batch_clean_sentences should be strings of shape (batch_size)
...
def _custom_bert_tokenize_sentence(text):
    ...
    # -----> assert len(split_sizes)==len(text.split()), print(len(tokens), len(split_sizes),
    #           len(text.split()), split_sizes, text)
...
def bert_tokenize(batch_sentences):
    ...
    # batch_encoded_dicts = [BERT_TOKENIZER.encode_plus(tokens,max_length=max_seq_len,pad_to_max_length=True)

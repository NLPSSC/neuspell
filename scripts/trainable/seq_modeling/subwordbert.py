...
def model_predictions(model, data, vocab, DEVICE, BATCH_SIZE=16):
    ...
    for batch_id, (batch_labels, batch_sentences) in enumerate(data_iter):
        ...
        batch_predictions = untokenize_without_unks(
            batch_predictions, batch_lengths, vocab, batch_labels
        )
...
if __name__ == "__main__":
    ...
    if TRAIN_NOISE_TYPE == "word":
        ...
        CHECKPOINT_PATH = os.path.join(
            BASE_PATH, "checkpoints/subwordbert-probwordnoise"
        )
    elif TRAIN_NOISE_TYPE == "probword_v2":
        CHECKPOINT_PATH = os.path.join(
            BASE_PATH, "checkpoints/subwordbert-probwordnoise_v2"
        )
    ...
    if TRAIN_NOISE_TYPE == "word":
        ...
        train_data = load_data(
            TRAIN_TEST_FILE_PATH, "train.1blm.v2", "train.1blm.v2.noise.probword"
        )
        train_data, valid_data = train_validation_split(train_data, 0.8, seed=11690)
        print(len(train_data), len(valid_data))
    elif TRAIN_NOISE_TYPE == "none":
    ...
    if START_EPOCH > 0:
        ...
        optimizer = BertAdam(
            optimizer_grouped_parameters, lr=5e-5, warmup=0.1, t_total=t_total
        )
        ...
        for epoch_id in range(START_EPOCH, N_EPOCHS + 1):
            ...
            for batch_id, (batch_labels, batch_sentences) in enumerate(train_data_iter):
                ...
                if len(batch_labels_) == 0:
                    print("################")
                    print(
                        "Not training the following lines due to pre-processing mismatch: \n"
                    )
                    print([(a, b) for a, b in zip(batch_labels, batch_sentences)])
                    print("################")
                    continue
                ...
                if batch_id % 10000 == 0:
                    ...
                    with torch.no_grad():
                        _, batch_predictions = model(
                            batch_bert_inp, batch_bert_splits, targets=batch_labels
                        )
                    model.train()
            ...
            for batch_id, (batch_labels, batch_sentences) in enumerate(valid_data_iter):
                ...
                if len(batch_labels_) == 0:
                    print("################")
                    print(
                        "Not validating the following lines due to pre-processing mismatch: \n"
                    )
                    print([(a, b) for a, b in zip(batch_labels, batch_sentences)])
                    print("################")
                    continue

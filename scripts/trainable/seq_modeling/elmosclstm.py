...
def get_losses_from_gpt_lm(
    this_sents: "list[str]", gpt2LMHeadModel, gpt2Tokenizer, DEVICE
):

    this_input_ids = gpt2Tokenizer.batch_encode_plus(
        this_sents,
    ...
    this_labels = torch.tensor(
        [
            [i if i != gpt2Tokenizer.pad_token_id else -100 for i in row]
            for row in this_input_ids
        ]
    ).to(DEVICE)
...
def get_losses_from_txl_lm(
    this_sents: "list[str]", txlLMHeadModel, txlTokenizer, DEVICE
):

    this_input_ids_dict = txlTokenizer.batch_encode_plus(
        this_sents,
        add_special_tokens=True,
        pad_to_max_length=True,
        add_space_before_punct_symbol=True,
    ...
    this_labels = torch.tensor(
        [
            [i if i != txlTokenizer.pad_token_id else -100 for i in row]
            for row in this_input_ids
        ]
    ).to(DEVICE)
    this_input_ids = torch.tensor(this_input_ids).to(DEVICE)
    ...
    losses = [
        sum(this_loss[str_pos : end_pos - 1])
        for str_pos, end_pos in zip([0] + chunks_cumsum[:-1], chunks_cumsum)
    ]
...
def load_pretrained(model, CHECKPOINT_PATH, optimizer=None, device="cuda"):
    ...
    checkpoint_data = torch.load(
        os.path.join(CHECKPOINT_PATH, "model.pth.tar"), map_location=map_location
    )
    ...
    print(
        f"previously, max_dev_acc: {max_dev_acc:.5f} and argmax_dev_acc: {argmax_dev_acc:.5f}"
    )
...
def model_predictions(
    model, data, vocab, DEVICE, BATCH_SIZE=16, backoff="pass-through"
):
    """
    ...
    model.eval()
    model.to(DEVICE)
    for batch_id, (batch_clean_sentences, batch_corrupt_sentences) in enumerate(
        data_iter
    ):
        # set batch data
        ...
        batch_elmo_inp = elmo_batch_to_ids(
            [line.split() for line in batch_corrupt_sentences]
        ).to(DEVICE)
        # forward
        with torch.no_grad():
            """
...
def model_predictions_for_ui(
    ...
    if beam_search:
        if topk < 2:
            raise Exception(
                "when using beam_search, topk must be greater than 1, topk is used as beam width"
            )
        else:
            print(f":: doing BEAM SEARCH with topk:{topk} ::")
    else:
        assert topk == 1, print(
            "if not beam_search, topk is set to 1 for UI-website purposes"
        )
    ...
    for batch_id, (batch_clean_sentences, batch_corrupt_sentences) in enumerate(
        ...
        if not beam_search:
            ...
            k_batch_predictions, k_batch_predictions_probs = untokenize_without_unks3(
                batch_predictions,
            ...
            final_sentences = [
                x[0] for x in k_batch_predictions
            ]  # a list with multiple answers
        print("*&$&%^$*^*&%")
...
def model_inference(
    ...
    model.eval()
    model.to(DEVICE)
    for batch_id, (batch_clean_sentences, batch_corrupt_sentences) in tqdm(
        enumerate(data_iter)
    ):
        ...
        if not beam_search:
            ...
            if topk == 1:
                batch_predictions = untokenize_without_unks(
                    batch_predictions, batch_lengths, vocab, batch_corrupt_sentences
                )
            else:
            ...
            corr2corr_, corr2incorr_, incorr2corr_, incorr2incorr_ = get_metrics(
                batch_clean_sentences,
            ...
            with torch.no_grad():
                for b in range(len(batch_clean_sentences)):
                    ...
                    losses = get_losses_from_gpt_lm(
                        this_sents, gpt2LMHeadModel, gpt2Tokenizer, DEVICE
                    )
            ...
            batch_predictions_k = [
                [k_batch_predictions[j][i] for j in range(len(k_batch_predictions))]
                for i in range(len(k_batch_predictions[0]))
            ]
            batch_predictions_probs_k = [
                [
                    k_batch_predictions_probs[j][i]
                    for j in range(len(k_batch_predictions_probs))
                ]
                for i in range(len(k_batch_predictions_probs[0]))
            ]
                ...
                results.append(
                    {
                        ...
                        "predicted": c,
                        "topk": d,
                        "topk_prediction_probs": e,
                        "topk_reranker_losses": f,
                    }
                )
            line_index += len(batch_clean_sentences)
    ...
    if not beam_search and selected_lines_file is not None:
        ...
        assert len(data) == len(predictions), print(
            len(data), len(predictions), "lengths mismatch"
        )
        ...
        if selected_lines_file is not None:
            selected_lines = {
                num: ""
                for num in [
                    int(line.strip()) for line in open(selected_lines_file, "r")
                ]
            }
        else:
            selected_lines = None

        clean_lines, corrupt_lines, predictions_lines = (
            [tpl[0] for tpl in data],
            [tpl[1] for tpl in data],
            predictions,
        )

        corr2corr, corr2incorr, incorr2corr, incorr2incorr, mistakes = get_metrics(
            clean_lines,
            corrupt_lines,
            predictions_lines,
...
if __name__ == "__main__":
    ...
    if START_EPOCH == 0:
        raise Exception(
            "START_EPOCH must be a non-zero value; If starting from scratch, use 1 instead of 0"
        )
    ...
    if len(sys.argv) > 4:
        ...
        if FINETUNE == "probword":
            SRC_CHECKPOINT_PATH = os.path.join(
                BASE_PATH, "checkpoints/elmoscrnn-probwordnoise"
            )
            SRC_VOCAB_PATH = os.path.join(SRC_CHECKPOINT_PATH, "vocab.pkl")
            print(
                f"Model finetuning with arg: {FINETUNE}, and source model selected from: {SRC_CHECKPOINT_PATH}"
            )
        else:
            raise Exception("only ```probword``` is now supported for finetuning")
        assert os.path.exists(SRC_CHECKPOINT_PATH), print(
            f"{SRC_CHECKPOINT_PATH} path unavailable"
        )
    else:
        FINETUNE = ""
    ...
    if TRAIN_NOISE_TYPE == "word":
        ...
        CHECKPOINT_PATH = os.path.join(
            BASE_PATH, "checkpoints/elmoscrnn-probwordnoise_v2"
        )
    elif TRAIN_NOISE_TYPE == "bea40kfinetune":
        CHECKPOINT_PATH = os.path.join(
            BASE_PATH, "checkpoints/elmoscrnn-probwordnoise-bea40kfinetune"
        )
    elif TRAIN_NOISE_TYPE == "moviereviewsfinetune":
        CHECKPOINT_PATH = os.path.join(
            BASE_PATH, "checkpoints/elmoscrnn-probwordnoise-moviereviewsfinetune2"
        )
        ...
        if FINETUNE != "":
            ...
            if TRAIN_NOISE_TYPE == "bea40kfinetune":
                train_data = load_data(
                    TRAIN_TEST_FILE_PATH, "train.bea40k", "train.bea40k.noise"
                )
                train_data, valid_data = train_validation_split(
                    train_data, 0.90, seed=11690
                )
                print(len(train_data), len(valid_data))
            elif TRAIN_NOISE_TYPE == "moviereviewsfinetune":
                #
                train_data_clean = get_lines(
                    os.path.join(TRAIN_TEST_FILE_PATH, "train.moviereviews")
                )
                train_data_noise1 = get_line_representation(
                    train_data_clean,
                    rep_list=["swap", "drop", "add", "key"],
                    probs=[0.25, 0.25, 0.25, 0.25],
                )
                train_data_noise2 = get_line_representation(
                    train_data_clean,
                    rep_list=["swap", "drop", "add", "key"],
                    probs=[1.00, 0.00, 0.00, 0.00],
                )
                train_data_noise3 = get_line_representation(
                    train_data_clean,
                    rep_list=["swap", "drop", "add", "key"],
                    probs=[0.00, 1.00, 0.00, 0.00],
                )
                train_data_noise4 = get_line_representation(
                    train_data_clean,
                    rep_list=["swap", "drop", "add", "key"],
                    probs=[0.00, 0.00, 1.00, 0.00],
                )
                train_data_noise5 = get_line_representation(
                    train_data_clean,
                    rep_list=["swap", "drop", "add", "key"],
                    probs=[0.00, 0.00, 0.00, 1.00],
                )
                train_data_noise = (
                    train_data_noise1
                    + train_data_noise2
                ...
                train_data = [
                    (a, b) for a, b in zip(train_data_clean, train_data_noise)
                ]
                #
                valid_data_clean = get_lines(
                    os.path.join(TRAIN_TEST_FILE_PATH, "valid.moviereviews")
                )
                valid_data_noise1 = get_line_representation(
                    valid_data_clean,
                    rep_list=["swap", "drop", "add", "key"],
                    probs=[0.25, 0.25, 0.25, 0.25],
                )
                valid_data_noise2 = get_line_representation(
                    valid_data_clean,
                    rep_list=["swap", "drop", "add", "key"],
                    probs=[1.00, 0.00, 0.00, 0.00],
                )
                valid_data_noise3 = get_line_representation(
                    valid_data_clean,
                    rep_list=["swap", "drop", "add", "key"],
                    probs=[0.00, 1.00, 0.00, 0.00],
                )
                valid_data_noise4 = get_line_representation(
                    valid_data_clean,
                    rep_list=["swap", "drop", "add", "key"],
                    probs=[0.00, 0.00, 1.00, 0.00],
                )
                valid_data_noise5 = get_line_representation(
                    valid_data_clean,
                    rep_list=["swap", "drop", "add", "key"],
                    probs=[0.00, 0.00, 0.00, 1.00],
                )
                ...
                valid_data = [
                    (a, b) for a, b in zip(valid_data_clean, valid_data_noise)
                ]
                print(len(train_data), len(valid_data))
            else:
            ...
            if TRAIN_NOISE_TYPE == "word":
                train_data = load_data(
                    TRAIN_TEST_FILE_PATH, "train.1blm", "train.1blm.noise.word"
                )
                train_data, valid_data = train_validation_split(
                    train_data, 0.8, seed=11690
                )
                print(len(train_data), len(valid_data))
            elif TRAIN_NOISE_TYPE == "prob":
                train_data = load_data(
                    TRAIN_TEST_FILE_PATH, "train.1blm", "train.1blm.noise.prob"
                )
                train_data, valid_data = train_validation_split(
                    train_data, 0.8, seed=11690
                )
                ...
                train_data = load_data(
                    TRAIN_TEST_FILE_PATH, "train.1blm", "train.1blm.noise.random"
                )
                train_data, valid_data = train_validation_split(
                    train_data, 0.8, seed=11690
                )
                ...
                train_data1, valid_data1 = train_validation_split(
                    train_data1, 0.8, seed=11690
                )
                print(len(train_data1), len(valid_data1))
                train_data2 = load_data(
                    TRAIN_TEST_FILE_PATH, "train.1blm", "train.1blm.noise.word"
                )
                train_data2, valid_data2 = train_validation_split(
                    train_data2, 0.8, seed=11690
                )
    ...
    if START_EPOCH > 0:
        ...
        if START_EPOCH > 1:
            ...
            progress_write_file = open(
                os.path.join(
                    CHECKPOINT_PATH, f"progress_retrain_from_epoch{START_EPOCH}.txt"
                ),
                "w",
            )
            ...
            if FINETUNE != "":
                ...
                model, optimizer, _, _ = load_pretrained(
                    model, SRC_CHECKPOINT_PATH, optimizer=optimizer
                )
                progress_write_file.write(
                    f"Training model params after loading from path: {SRC_CHECKPOINT_PATH}\n"
                )
            else:
                print(f"loading pretrained weights from {CHECKPOINT_PATH}")
                model, optimizer, max_dev_acc, argmax_dev_acc = load_pretrained(
            ...
            # file to write progress to
            progress_write_file = open(
                os.path.join(CHECKPOINT_PATH, "progress.txt"), "w"
            )
            print("Training model params from scratch")
            progress_write_file.write("Training model params from scratch\n")
        ...
        for epoch_id in range(START_EPOCH, N_EPOCHS + 1):
            ...
            if (epoch_id - argmax_dev_acc) > patience:
                ...
                print(
                    "max_dev_acc: {}, argmax_dev_acc: {}".format(
                        max_dev_acc, argmax_dev_acc
                    )
                )
                break
            ...
            if TRAIN_NOISE_TYPE == "moviereviewsfinetune":
                ...
                print(
                    f"new training instances created, train data size now: {len(train_data)}"
                )
            # print epoch
            ...
            for batch_id, (batch_labels, batch_sentences) in enumerate(train_data_iter):
                ...
                batch_labels = batch_labels.to(DEVICE)
                batch_elmo_inp = elmo_batch_to_ids(
                    [line.split() for line in batch_sentences]
                ).to(DEVICE)
                # forward
                model.train()
                loss = model(
                    batch_idxs, batch_lengths, batch_elmo_inp, targets=batch_labels
                )
            ...
            try:
                ...
                progress_write_file.flush()
                valid_data_iter = batch_iter(
                    valid_data, batch_size=VALID_BATCH_SIZE, shuffle=False
                )
                for batch_id, (batch_labels, batch_sentences) in enumerate(
                    valid_data_iter
                ):
                    st_time = time.time()
                    # set batch data
                ...
                if valid_acc >= max_dev_acc:
                    ...
                    print(
                        "Model saved at {} in epoch {}".format(
                            os.path.join(CHECKPOINT_PATH, name), epoch_id
                        )
                    )
                ...
                print(
                    "Model saved at {} in epoch {}".format(
                        os.path.join(temp_folder, name), epoch_id
                    )
                )
                raise Exception(e)
    else:
        ...
        for line in greedy_results:
            opfile.write(line)
        opfile.close()
        opfile = jsonlines.open(
            os.path.join(ANALYSIS_DIR, "greedy_results_corr_preds.jsonl"), "w"
        )
        for line in [
            line for line in greedy_results if line["original"] == line["predicted"]
        ]:
            opfile.write(line)
        opfile.close()
        opfile = jsonlines.open(
            os.path.join(ANALYSIS_DIR, "greedy_results_incorr_preds.jsonl"), "w"
        )
        for line in [
            line for line in greedy_results if line["original"] != line["predicted"]
        ]:
        ...
        opfile.close()
        opfile = open(
            os.path.join(ANALYSIS_DIR, "greedy_results_incorr_preds.txt"), "w"
        )
        for line in [
            line for line in greedy_results if line["original"] != line["predicted"]
        ]:

...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 4:
        ...
        if FINETUNE == "probword":
            SRC_CHECKPOINT_PATH = os.path.join(
                BASE_PATH, "checkpoints/lstm-lstm-probwordnoise"
            )
    ...
    if TRAIN_NOISE_TYPE == "word":
        ...
        CHECKPOINT_PATH = os.path.join(
            BASE_PATH, "checkpoints/lstm-lstm-probwordnoise_v2"
        )
    elif TRAIN_NOISE_TYPE == "bea40kfinetune":
        CHECKPOINT_PATH = os.path.join(
            BASE_PATH, "checkpoints/lstm-lstm-probwordnoise-bea40kfinetune"
        )
    elif TRAIN_NOISE_TYPE == "moviereviewsfinetune":
        CHECKPOINT_PATH = os.path.join(
            BASE_PATH, "checkpoints/lstm-lstm-probwordnoise-moviereviewsfinetune2"
        )
    ...
    if START_EPOCH > 0:
        ...
        for epoch_id in range(START_EPOCH, N_EPOCHS + 1):
            ...
            for batch_id, (batch_labels, batch_sentences) in enumerate(train_data_iter):
                ...
                model.train()
                loss = model(
                    batch_idxs, batch_char_lengths, batch_lengths, targets=batch_labels
                )

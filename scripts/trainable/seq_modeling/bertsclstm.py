...
def model_inference(model, data, topk, DEVICE, BATCH_SIZE=16, vocab_=None):
    ...
    print(
        (
            f"_corr2corr:{_corr2corr}, _corr2incorr:{_corr2incorr}, "
            f"_incorr2corr:{_incorr2corr}, _incorr2incorr:{_incorr2incorr}"
        )
    )
    print(
...
if __name__ == "__main__":
    ...
    if START_EPOCH > 0:
        ...
        lr = 1e-4  # lr = 5e-5
        optimizer = BertAdam(
            optimizer_grouped_parameters, lr=lr, warmup=0.1, t_total=t_total
        )

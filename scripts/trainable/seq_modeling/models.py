...
ROOT_URL = "https://allennlp.s3.amazonaws.com/models/elmo/2x4096_512_2048cnn_2xhighway"
...
class CharCNNWordLSTMModel(nn.Module):
    ...
    def forward(
        self,
        batch_idxs: "list[pad_sequence]",  # noqa: F821
        batch_lengths: "tensor",  # noqa: F821
        aux_word_embs: "tensor" = None,  # noqa: F821
        targets: "tensor" = None,  # noqa: F821
        topk=1,
    ):
...
class CharLSTMModel(nn.Module):
    ...
    def forward(self, batch_tensor, batch_lengths):
        ...
        # lstm
        # dim: [BS,max_nwords,*]->[BS,max_nwords,self.lstmmodule_outdim]
        embs_packed = pack_padded_sequence(
            embs, batch_lengths, batch_first=True, enforce_sorted=False
        )
        lstm_encodings, (last_hidden_states, last_cell_states) = self.lstmmodule(
            embs_packed
        )
        ...
        if self.output_combination == "end":
            ...
            source_encodings = lstm_encodings[
                range(lstm_encodings.shape[0]), last_seq_idxs, :
            ]
            ...
            lens_ = batch_lengths.unsqueeze(dim=1).expand(
                batch_size, self.lstmmodule_outdim
            )
...
class CharLSTMWordLSTMModel(nn.Module):
    def __init__(self, nchars, char_emb_dim, char_padding_idx, padding_idx, output_dim):
        ...
        self.charlstmmodule = CharLSTMModel(
            nchars,
            char_emb_dim,
            char_padding_idx,
            hidden_size,
    ...
    def forward(
        self,
        batch_idxs: "list[pad_sequence]",  # noqa: F821
        batch_char_lengths: "list[tensor]",  # noqa: F821
        batch_lengths: "tensor",  # noqa: F821
        aux_word_embs: "tensor" = None,  # noqa: F821
        targets: "tensor" = None,  # noqa: F821
        topk=1,
        ...
        # charlstm
        charlstm_encodings = [
            self.charlstmmodule(pad_sequence_, lens)
            for pad_sequence_, lens in zip(batch_idxs, batch_char_lengths)
        ]
        charlstm_encodings = pad_sequence(
            charlstm_encodings, batch_first=True, padding_value=0
        )
...
class SCLSTM(nn.Module):
    ...
    def forward(
        self,
        batch_screps: "list[pad_sequence]",  # noqa: F821
        batch_lengths: "tensor",  # noqa: F821
        aux_word_embs: "tensor" = None,  # noqa: F821
        targets: "tensor" = None,  # noqa: F821
        topk=1,
...
class ElmoSCLSTM(nn.Module):
    ...
    def __init__(self, screp_dim, padding_idx, output_dim, early_concat=True):
        ...
        options_file = f"{ROOT_URL}/elmo_2x4096_512_2048cnn_2xhighway_options.json"
        weight_file = f"{ROOT_URL}/elmo_2x4096_512_2048cnn_2xhighway_weights.hdf5"
        self.elmo = Elmo(
            options_file, weight_file, 1
        )  # 1 for setting device="cuda:0" else 0
    ...
    def forward(
        ...
        # elmo
        elmo_encodings = self.elmo(elmo_inp)["elmo_representations"][
            0
        ]  # BS X max_nwords x 1024
        ...
        if not self.training:
            ...
            if not beam_search:
                if topk > 1:
                    topk_probs, topk_inds = torch.topk(
                        probs, topk, dim=-1, largest=True, sorted=True
                    )  # -> (Tensor, LongTensor) of [BS,max_nwords,topk]
                elif topk == 1:
                    topk_inds = torch.argmax(probs, dim=-1)  # [BS,max_nwords]
                ...
                return (
                    ...
                    topk_inds.cpu().detach().numpy(),
                    topk_probs.cpu().detach().numpy(),
                )
...
class SubwordElmo(nn.Module):
    ...
    def forward(
        self,
        batch_elmo_inp: "tensor",  # noqa: F821
        aux_word_embs: "tensor" = None,  # noqa: F821
        targets: "tensor" = None,  # noqa: F821
        topk=1,
        beam_search=False,
...
- See https://pytorch.org/docs/stable/nn.html#transformerencoder for multiple encoder layers
    ...
    - See https://github.com/pytorch/pytorch/blob/a5b509985a37127fb52fbfdee85c7b336cd8d2c1
        /torch/nn/modules/activation.py#L799
...
class ElmoSCTransformer(nn.Module):
    def __init__(self, screp_dim, padding_idx, output_dim):
        ...
        encoder_layer = torch.nn.TransformerEncoderLayer(
            d_model=self.transmodule_indim,
            nhead=nhead,
    ...
    def forward(
        self,
        batch_screps: "list[pad_sequence]",  # noqa: F821
        src_key_padding_mask: "tensor",  # noqa: F821
        batch_lengths: "tensor",  # noqa: F821
        elmo_inp: "tensor",  # noqa: F821
        aux_word_embs: "tensor" = None,  # noqa: F821
        targets: "tensor" = None,  # noqa: F821
        ...
        transformer_encodings = self.transmodule(
            intermediate_encodings, src_key_padding_mask=src_key_padding_mask
        )
...
class BertSCLSTM(nn.Module):
    ...
    def forward(
        ...
        batch_screps: "list[pad_sequence]",  # noqa: F821
        batch_lengths: "tensor",  # noqa: F821
        batch_bert_dict: "{'input_ids':tensor, 'attention_mask':tensor, 'token_type_ids':tensor}",  # noqa: F821
        batch_splits: "list[list[int]]",  # noqa: F821
        aux_word_embs: "tensor" = None,  # noqa: F821
        targets: "tensor" = None,  # noqa: F821
        topk=1,
    ):
        ...
        if self.early_concat:
            ...
            # if not None, the expected dim for aux_word_embs: [BS,max_nwords,*]
            intermediate_encodings = torch.cat(
                (batch_screps, bert_merged_encodings), dim=2
            )
            if aux_word_embs is not None:
                intermediate_encodings = torch.cat(
                    (intermediate_encodings, aux_word_embs), dim=2

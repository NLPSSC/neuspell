...
# simple usage
# -----
# python evals.py ../../data/traintest/test.bea4k ../../data/traintest/test.bea4k.noise \
#   ./gec-pseudodata/test.bea4k.predictions.txt ./gec-pseudodata test.bea4k.lines.txt
# python evals.py ../../data/traintest/test.bea60k ../../data/traintest/test.bea60k.noise \
#   ./gec-pseudodata/test.bea60k.predictions.txt ./gec-pseudodata test.bea60k.lines.txt
# python evals.py ../../data/traintest/test.bea20k ../../data/traintest/test.bea20k.noise \
#   ./gec-pseudodata/test.bea20k.predictions.txt ./gec-pseudodata test.bea20k.lines.txt
# python evals.py ../../data/traintest/test.jfleg ../../data/traintest/test.jfleg.noise \
#   ./gec-pseudodata/test.jfleg.predictions.txt ./gec-pseudodata test.jfleg.lines.txt
# python evals.py ../../data/traintest/test.1blm ../../data/traintest/test.1blm.noise.prob \
#   ./gec-pseudodata/test.1blm.noise.prob.predictions.txt ./gec-pseudodata test.1blm.noise.prob.lines.txt
# python evals.py ../../data/traintest/test.1blm ../../data/traintest/test.1blm.noise.word \
#   ./gec-pseudodata/test.1blm.noise.word.predictions.txt ./gec-pseudodata test.1blm.word.prob.lines.txt
# -----
# usage w/ selected lines
# -----
# python evals.py ../../data/traintest/test.bea4k ../../data/traintest/test.bea4k.noise \
#   ./gec-pseudodata/test.bea4k.predictions.txt ./gec-pseudodata test.bea4k.lines.txt \
#   ./gec-pseudodata/test.bea4k.lines.txt
# python evals.py ../../data/traintest/test.bea60k ../../data/traintest/test.bea60k.noise \
#   ./gec-pseudodata/test.bea60k.predictions.txt ./gec-pseudodata test.bea60k.lines.txt \
#   ./gec-pseudodata/test.bea60k.lines.txt
# python evals.py ../../data/traintest/test.bea20k ../../data/traintest/test.bea20k.noise \
#   ./gec-pseudodata/test.bea20k.predictions.txt ./gec-pseudodata test.bea20k.lines.txt \
#   ./gec-pseudodata/test.bea20k.lines.txt
# python evals.py ../../data/traintest/test.jfleg ../../data/traintest/test.jfleg.noise \
#   ./gec-pseudodata/test.jfleg.predictions.txt ./gec-pseudodata test.jfleg.lines.txt \
#   ./gec-pseudodata/test.jfleg.lines.txt
#####################################
...
def __get_data(inp):
    if isinstance(inp, list):
        ...
        raise Exception(
            "arg inp must be either clean_data or a path to fetch clean_data"
        )
    return data
...
def get_metrics(
    ...
    if isinstance(predictions[0][0], str):
        is_correct_prediction = (
            lambda clean_token, corrupt_token: clean_token == corrupt_token
        )
    elif isinstance(predictions[0][0], list):
        is_correct_prediction = (
            lambda clean_token, preds_list: clean_token in preds_list[:check_until_topk]
        )
    else:
    ...
    if selected_lines is not None:
        assert isinstance(selected_lines, dict), print(
            f"{type(selected_lines)} typefound when expecting type dict"
        )
        print(
            f"evaluating only for selected lines: {len(selected_lines)}/{len(clean_data)}"
        )
    for i, (clean_line, corrupt_line, predictions_) in enumerate(
        zip(clean_data, corrupt_data, predictions)
    ):
        if selected_lines is not None:
            if i not in selected_lines:
                continue

        clean_line_tokens, corrupt_line_tokens = (
            clean_line.split(),
            corrupt_line.split(),
        )
        ...
        #   or a list of (len(clean_line_tokens),) tokens
        ...
        for i, (clean_token, corrupt_token, prediction_tokens) in enumerate(
            ...
            if clean_token == corrupt_token and is_correct_prediction(
                clean_token, prediction_tokens
            ):
                corr2corr += 1
            elif clean_token == corrupt_token and not is_correct_prediction(
                clean_token, prediction_tokens
            ):
                corr2incorr += 1
            elif clean_token != corrupt_token and is_correct_prediction(
                clean_token, prediction_tokens
            ):
                incorr2corr += 1
            elif clean_token != corrupt_token and not is_correct_prediction(
                    ...
                    mistakes.append(
                        (
                            ...
                            " ".join(
                                corrupt_line_tokens[
                                    max(i - window, 0) : min(
                                        i + window + 1, len(corrupt_line_tokens)
                                    )
                                ]
                            ),
                        )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 6:
        selected_lines_file = sys.argv[
            6
        ]  # only evaluate for a selected set of line numbers in the predictions
    else:
    ...
    for line in opfile:
        corrupt_lines.append(line.strip())
    opfile.close()

    predictions_lines = []
    opfile = open(predictions_file_path, "r")
    for line in opfile:
        predictions_lines.append(line.strip())
    opfile.close()
    ...
    if selected_lines_file is not None:
        selected_lines = {
            num: ""
            for num in [int(line.strip()) for line in open(selected_lines_file, "r")]
        }
        ...
        opfle = open(
            os.path.join(evaluated_line_nums_savedir, evaluated_line_nums_savename), "w"
        )
        for num in nlines:
            opfle.write(f"{num}\n")
        opfle.close()
...
def get_metrics2(clean,corrupt,predictions,return_mistakes=False,window=7):
    ...
    for clean_line,corrupt_line,predictions_ in zip(clean_data,corrupt_data,predictions):
        ...
        for i, (clean_token,corrupt_token,prediction_tokens) in \
                enumerate( zip(clean_line_tokens, corrupt_line_tokens, predictions_) ):
            if clean_token==corrupt_token and clean_token in prediction_tokens:
                ...
                incorr2incorr+=1
                #if return_mistakes: mistakes.append( \
                #   (" ".join(clean_line_tokens[max(i-window,0):min(i+window+1,len(clean_line_tokens))]),
                #                                      corrupt_token,
                #                                      prediction_tokens) )
                if return_mistakes: mistakes.append( \
                        (clean_token,corrupt_token,prediction_tokens, \
...
def get_metrics(clean,corrupt,prediction,return_mistakes=False):
    ...
    for clean_line,corrupt_line,prediction_line in tqdm(zip(clean_data,corrupt_data,prediction_data)):
        clean_line_tokens, corrupt_line_tokens, prediction_line_tokens = \
            clean_line.split(),corrupt_line.split(),prediction_line.split()
        for i, (clean_token,corrupt_token,prediction_token) in \
                enumerate( zip(clean_line_tokens, corrupt_line_tokens, prediction_line_tokens) ):

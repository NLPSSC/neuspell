...
class ContextProbabilityBERTLM(ContextProbability):
    ...
    def _load_bert_model(self, path):
        try:
            ...
            print(
                "Unable to load model and tokenizer data from local path: \
                 {}; Loading from huggingface site".format(
                    path
                )
            )
    ...
    def _get_masked_sents(
        self, tokenized_sentences, indices, add_terminal_tokens=False
    ):
        ...
        for tsent, ind in zip(tokenized_sentences, indices):
            ...
            msent = (
                "{} {} {}".format(self.__cls_token, " ".join(tsent), self.__sep_token)
                if add_terminal_tokens
                else " ".join(tsent)
            )
            masked_sents.append(msent)
        return masked_sents

    def __addto_forward_pass_stacklist(
        self,
        wtsent,
        masked_sents,
        tsent_seq_len,
        ...
        if potential_msent in masked_sents:
            ...
            tsent_mask_index.append(
                dummy1.index(self.__mask_token)
            )  # returns first index of self.__mask_token if multiple exists
            tsent_seq_len.append(len(dummy1))
            candidate_map[i][candidate].append(
                (len(masked_sents) - 1, self.vocab[vocab_search_candidate])
            )
        return masked_sents, tsent_seq_len, tsent_mask_index, candidate_map

    def _get_bert_probabilities(
        self, tokenized_sentences, indices, candidates=[], max_seq_len=-1
        ...
        if len(candidates) > 0:
            ...
            candidate_map = {}
            for i, (wtsent, wtind, row) in enumerate(
                zip(tokenized_sentences, indices, candidates)
            ):
                candidate_map[i] = {}
                for candidate in row:
                    ...
                    if candidate in self.vocab:
                        ...
                        masked_sents, tsent_seq_len, tsent_mask_index, candidate_map = (
                            self.__addto_forward_pass_stacklist(
                                wtsent_copy,
                                masked_sents,
                                tsent_seq_len,
                                tsent_mask_index,
                        ...
                        # if broken_candidates = [c1, c2, c3, c4], we have four potential_msents
                        # with different masking scheme
            ...
            assert len(tokenized_sentences) == len(indices)
            masked_sents = self._get_masked_sents(
                tokenized_sentences, indices, add_terminal_tokens=True
            )
        ...
        while batch_start < nsents:
            ...
            batch_masked_sents = masked_sents[
                batch_start : min(batch_start + BATCH_SIZE, nsents)
            ]
            batch_tsent_seq_len = tsent_seq_len[
                batch_start : min(batch_start + BATCH_SIZE, nsents)
            ]
            batch_tsent_mask_index = tsent_mask_index[
                batch_start : min(batch_start + BATCH_SIZE, nsents)
            ]
            ...
            if (
                max_seq_len != -1
                and len(
                    [*filter(lambda x: x <= 0, max_seq_len - batch_tsent_seq_len - 2)]
                )
                == 0
            ):
            ...
            with torch.no_grad():
                batch_final_layer_logits = self.model(batch_input_ids)[
                    0
                ]  # [BATCH_SIZE,MAX_SEQUENCE_LEN,32k]

            batch_final_layer_logits = batch_final_layer_logits.to("cpu")
            for _final_layer_logits, _tsent_mask_index in zip(
                batch_final_layer_logits, batch_tsent_mask_index
            ):
                curr_softmax_scores = self._softmax(
                    _final_layer_logits[_tsent_mask_index, :].reshape(-1)
                )
        ...
        for i, candidate_list in enumerate(return_candidate_instances):
            return_candidate_instances[i] = sorted(
                candidate_list, key=lambda unit_: unit_.prob, reverse=True
            )
...
class SentenceProbability:
    ...
    def get_sentence_probabilities(self, sentences: List[List[str]]) -> torch.Tensor:
        ...
        padded_mask_tensor = (
            pad_sequence(input_mask, batch_first=True).float().to(self.device)
        )

        with torch.no_grad():
            prediction_scores, _ = self.model(padded_input_tensor)[:2]

        expanded_input_tensor = one_hot(
            padded_input_tensor, num_classes=self.NUM_CLASSES
        ).float()
        word_wise_probabilities = torch.mul(
            expanded_input_tensor, prediction_scores
        ).sum(dim=2)

        masked_word_wise_probabilities = torch.mul(
            word_wise_probabilities, padded_mask_tensor
        )
    ...
    def get_candidate_probabilities(
        ...
        for sentence, index, candidate_list in zip(sentences, indices, candidates):
            ...
            all_probabilities.append(
                self.get_sentence_probabilities(candidate_sentence_list)
            )

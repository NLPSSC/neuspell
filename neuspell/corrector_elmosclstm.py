...
if is_module_available("allennlp"):
    from .seq_modeling.elmosclstm import (
        load_model,
        load_pretrained,
        model_predictions,
        model_inference,
    ...
    def finetune(
        ...
        for epoch_id in range(START_EPOCH, N_EPOCHS + 1):
            ...
            for batch_id, (batch_labels, batch_sentences) in enumerate(train_data_iter):
                ...
                elmo_batch_to_ids = get_module_or_attr(
        ...
        print(
            f"Model and logs saved at {os.path.join(CHECKPOINT_PATH, 'model.pth.tar')}"
        )
        return

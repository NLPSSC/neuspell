...
class BertChecker(Corrector):
    ...
    def correct_strings(self, mystrings: List[str], return_all=False) -> List[str]:
        self.is_model_ready()
        mystrings = bert_tokenize_for_valid_examples(
            mystrings, mystrings, self.bert_pretrained_name_or_path
        )[0]
    ...
    def from_huggingface(self, bert_pretrained_name_or_path, vocab: Union[Dict, str]):
        ...
        if isinstance(vocab, str) and os.path.exists(vocab):
            ...
            raise ValueError(
                f"unknown vocab type or unable to find path: {type(vocab)}"
            )
        self.model = load_model(
            self.vocab, bert_pretrained_name_or_path=self.bert_pretrained_name_or_path
        )
    ...
    def finetune(
        ...
        CHECKPOINT_PATH = os.path.join(
            self.ckpt_path if self.ckpt_path else data_dir,
            "new_models",
        ...
        if START_EPOCH > 1:
            ...
            print("Training model params")
            progress_write_file.write("Training model params\n")
        ...
        for epoch_id in range(START_EPOCH, N_EPOCHS + 1):
            ...
            for batch_id, (batch_labels, batch_sentences) in enumerate(train_data_iter):
                ...
                batch_labels_, batch_sentences_, batch_bert_inp, batch_bert_splits = (
                    bert_tokenize_for_valid_examples(
                        batch_labels, batch_sentences, self.bert_pretrained_name_or_path
                    )
                )
            ...
            if valid_acc >= max_dev_acc:
                print(
                    f"validation accuracy improved from {max_dev_acc:.4f} to {valid_acc:.4f}"
                )

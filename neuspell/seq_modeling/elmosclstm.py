from neuspell.seq_modeling.helpers import untokenize_without_unks2
from neuspell.seq_modeling.helpers import untokenize_without_unks3
...
def load_pretrained(model, checkpoint_path, optimizer=None, device="cuda"):
    if optimizer:
        raise Exception(
            "If you want optimizer, call `load_pretrained_large(...)` instead of `load_pretrained(...)`"
        )
...
def load_pretrained_large(model, checkpoint_path, optimizer=None, device="cuda"):
    ...
    print(f"Loading model params from checkpoint dir: {checkpoint_path}")
    checkpoint_data = torch.load(
        os.path.join(checkpoint_path, "model.pth.tar"),
        map_location=map_location,
        weights_only=False,
    )
...
def model_inference(
    ...
    inference_st_time = time.time()
    VALID_batch_size = batch_size
    valid_loss, valid_acc = 0.0, 0.0  # noqa: F841

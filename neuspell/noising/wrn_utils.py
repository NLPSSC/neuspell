...
def _calculate_mistaketoken_overlap(
    original_sentences, mistakes_vocab, return_mode=False
):
    """
    find overlap
    ...
    print(
        "overlap percent wrt mistakes_vocab: {:.4f}".format(
            100 * len(overlap_words) / len(mistakes_vocab)
        )
    )
    if return_mode:
...
def noisyfy_word_tokens(
    ...
    print(
        "total lines in inp to noisyfy_word_tokens: {}".format(len(original_sentences))
    )
    print(
        "total tokens in inp to noisyfy_word_tokens: {}".format(
            sum([len(line.strip().split()) for line in original_sentences])
        )
    )
    # print("------------------------------------")
    overlap_words, overlap_count, total_count, overlap_percent = (
        _calculate_mistaketoken_overlap(
            original_sentences, mistakes_vocab, return_mode=True
        )
    )
    ...
    print(
        "{:.4f}% of overlapped tokens will get replaced to "
        "match the total % of misspellings to {:.4f}%".format(
            100 * prob, 100 * expected_prob
        )
    )

    get_noisy_token = lambda token: (
        random.choice(mistakes_mappings[token])
        if (
            token in mistakes_vocab
            ...
            for a, b in zip(error.split(), original.split()):
                if a != b:
                    try:
                        ...
                        diffvocabdict[b] = 1
        print(
            "No of tokens in mistakes_mappings queried: {}".format(
                len(
                    set([*mistakes_mappings.keys()]).intersection(
                        set([*diffvocabdict.keys()])
                    )
                )
            )
        )

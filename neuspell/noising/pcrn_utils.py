...
def get_lcs(str1: Literal["correct word"], str2: Literal["error word"]):
...
def __get_dp(w_c, w_e, i_c, i_e, dp_counts, dp_changes):
    ...
    if i_e == -1 and i_c == -1:
        return [], 0
    if i_e == -1 and i_c >= 0:
        return [(i, w_c[i], "") for i in range(i_c, -1, -1)], i_c + 1

    if dp_counts[i_c][i_e] == -np.inf:  # need to fill
        if w_c[i_c] == w_e[i_e]:
            dp_changes[i_c][i_e], dp_counts[i_c][i_e] = __get_dp(
                w_c, w_e, i_c - 1, i_e - 1, dp_counts, dp_changes
            )
        else:  # replace with char or replace will null in w_c as l_c>=l_e
            case1_changes, case1_count = __get_dp(
                w_c, w_e, i_c - 1, i_e - 1, dp_counts, dp_changes
            )
            case2_changes, case2_count = __get_dp(
                w_c, w_e, i_c - 1, i_e, dp_counts, dp_changes
            )
            if case1_count <= case2_count:
                dp_changes[i_c][i_e], dp_counts[i_c][i_e] = (
                    case1_changes + [(i_c, w_c[i_c], w_e[i_e])],
                    case1_count + 1,
                )
            else:
                dp_changes[i_c][i_e], dp_counts[i_c][i_e] = (
                    case2_changes + [(i_c, w_c[i_c], "")],
                    case2_count + 1,
                )
    return dp_changes[i_c][i_e], dp_counts[i_c][i_e]


def __get_replace_probs(
    stats,
    context_end_tokens,
    correct_char,
    context_length_category,
    return_sorted_list=False,
):
    try:
...
def __replace_only_topk(true_chars, mod_chars, mod_probs, top_k):
    ...
    for i, (true_char, mod_char, mod_prob) in enumerate(
        zip(true_chars, mod_chars, mod_probs)
    ):
        if mod_char != true_char:
            _mod_inds.append(i)
            _mod_probs.append(mod_prob)
    if not len(_mod_inds) == 0:
        for ind_ in np.random.choice(
            _mod_inds,
            min(top_k, len(_mod_inds)),
            replace=False,
            p=__sum_to_one(_mod_probs),
        ):
...
def _get_replace_probs_all_contexts(
    stats,
    raw_context,
    is_beginning,
    correct_char,
    ...
    for ln in range(0, len(raw_context) + 1):  # [0,1,2,3,...,len(context)]
        ...
        if alpha != 0:
            sum_alpha += alpha
            selected_raw_context_new = (
                append_left_start(selected_raw_context)
                if (ln == 0 and is_beginning)
                else selected_raw_context
            )
            probs_dict = __get_replace_probs(
                stats, selected_raw_context_new, correct_char, selected_raw_context_len
            )
            if print_stats:
                print(
                    sorted(probs_dict.items(), key=operator.itemgetter(1), reverse=True)
                )
    ...
    if print_stats:
        ...
        replace_dict_sorted = sorted(
            replace_dict.items(), key=operator.itemgetter(1), reverse=True
        )
        print(replace_dict_sorted)
        replace_me_with = []
        for _ in range(20):
            replace_char = np.random.choice(
                [chr(p) for p in range(nascii)] + [""], p=replace_char_probs
            )
        ...
        replace_char_prob = (
            replace_char_probs[ord(replace_char)]
            if replace_char != ""
            else replace_char_probs[epsilon_index]
        )
    return replace_char, replace_char_prob


def noisyfy_backoff_homophones(
    stats, lines, alphas, homophones={}, topk=-1, lower=False, print_data=False
):
    """
    ...
    print(
        "total tokens in inp to noisyfy_backoff_homophones: {}".format(
    ...
    for lll, line in tqdm(enumerate(lines)):
        ...
        for ttt, token in enumerate(wtokens):
            ...
            if token in homophones_set and np.random.rand() <= 0.1:
                ...
                if print_data:
                    print(lll, token, wtokens[ttt], " <-- homophone")
                _nchanges = (
                    get_lcs(token, wtokens[ttt])[1]
                    if len(token) >= len(wtokens[ttt])
            ...
            if top_k == 0:
                if len(token) <= 3:
                    ...
                    top_k = np.random.choice(
                        [0, 1, 2, 3, 4], p=[0.20, 0.25, 0.35, 0.15, 0.05]
                    )
            ...
            for i, char in enumerate(token):
                ...
                replace_char, replace_char_prob = _get_replace_probs_all_contexts(
                    stats,
                    token[start_:i],
                    True if start_ == 0 else False,
                    token[i],
                    alphas,
                    print_stats=False,
                )
                ...
                if char != replace_char:
                    _nchanges += 1
            if top_k >= 0:
                replace_token, _nchanges = __replace_only_topk(
                    true_chars, mod_chars, mod_probs, top_k
                )
            else:
        ...
        if print_data:
            print("corrupted line --> ", new_lines[lll])

    if print_data:
        print(
            f"total observed characters: {nchars}, \
               total corrupted characters: {nchanges}, \
               percent corrupted: {100 * nchanges / nchars}"
        )
        print(
            f"total observed words: {nwords}, \
